...
# flat alphabetical dump (design §1, §6). Ordered SETUP → DEVELOP → SHIP, matching the
...
def _getting_started_epilog() -> str:
    """The root's numbered "Getting started" path (design §5): login → init → cd → dev → deploy.
...
def _dim_capabilities_block(help_text: str) -> str:
    """Gray the root docstring's `\\b` capabilities block (Models/Tools/…) as secondary text.

    That block is a reference list, so it recedes like the command descriptions do — but it lives in
    the docstring, which the formatter leaves at full contrast. Dim the lines of the single `\\b`
    verbatim block (marked by a lone ``\\x08``) up to the next blank line, preserving each line's
    leading indent so alignment and Click's dedent are unaffected.
    """
    out: list[str] = []
    dimming = False
