...
async def mcp_tools(
    ...
    *,
    ...
    agent builds itself. Configured failures propagate; optional customer servers retain their
    existing identity and best-effort discovery behavior.
    """
    ...
    if servers:
        ...
        for server in servers:
            try:
                result.extend(await client.get_tools(server_name=server.name))
            except Exception as error:
                raise _auth_error(error, server.name) or AuthError(
                    "MCP_TOOL_FAILED", "Could not discover configured MCP tools.", 502, server.name
                ) from None
    if extra_servers:
        try:
            result.extend(await mcp_client(extra_servers, tools=()).get_tools())
        except Exception:
            logger.warning("Failed to fetch optional MCP tools; continuing without them.")
    return result
