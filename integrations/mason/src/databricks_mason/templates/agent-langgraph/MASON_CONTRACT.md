...
Metadata alone does not integrate adapters. Install the current `databricks-mason[runtime]`
distribution, supply the real command in `app.yaml`, load configuration before adapters, and listen
on the app port. The CLI and runtime find `agent.toml` from the working directory or
