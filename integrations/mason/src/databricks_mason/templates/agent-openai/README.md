...
`runtime/main.py` registers the agent's `invoke` and `recover` hooks with Mason Runtime. OpenAI
Agents SDK does not currently expose LangGraph-style node checkpoints, so recovery replays the
persisted application input against the same session. Deployed agents use a persistent Runtime
Store, so invocation state and emitted events survive process loss, but tool calls and other
external side effects remain at-least-once and must be idempotent. `mason dev` uses an in-process
Runtime Store and does not recover work after the process exits.
