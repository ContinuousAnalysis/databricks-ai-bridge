...
class _RequestStream(StreamingResponse):
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        try:
            ...
            await self.execution._finish(self.private_id, self.auth)
            self.execution._streams[self.private_id] -= 1
            if not self.execution._streams[self.private_id]:
                del self.execution._streams[self.private_id]
...
class RequestExecution:
    ...
    async def _finish(self, private_id: str, auth: RequestAuthContext) -> None:
        auth.close()
        ...
        try:
            ...
            if claimed is not None:
                ...
                try:
                    ...
                    await self._finish(private_id, auth)
