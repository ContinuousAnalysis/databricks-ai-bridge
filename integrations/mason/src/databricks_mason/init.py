...
#
# `versioned` templates live in this repo and are released in lockstep with the CLI: a scaffold
...
def _template_ref(spec: dict) -> str:
    """The git ref to fetch a framework's template from, absent a `--ref` override.

    For a `versioned` template, fetch the tag matching the installed CLI so the scaffold's pinned
    `databricks-mason` matches what the user has. Fall back to the spec's default ref when the
    version isn't a published release — an editable/dev build (e.g. `0.1.0.dev0`, or a local `+`
