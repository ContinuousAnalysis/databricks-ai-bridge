...
def dev(
    ...
    # Run against a local-only manifest that forces the Runtime Store in-process and removes deploy-only
...
def _dev_entry_point(app_yaml: pathlib.Path) -> pathlib.Path:
    ...
    The manifest marks the process as local so the Runtime Store uses in-memory state. Keeping this in
