...
`mason init` writes that pin (a git repo, a local editable checkout, or a bundled wheel) and
`mason deploy` reads it (and rewrites it to a bundled wheel). This module owns the pin's shapes and
the tomlkit plumbing so init and deploy don't each reimplement them.
"""
...
def editable(mason_dir: pathlib.Path) -> dict:
    ...
    Resolves only on this machine, so `mason deploy` rejects it (or bundles a wheel instead).
    """
    return {"path": str(mason_dir.resolve()), "editable": True}


def wheel(relative_path: str) -> dict:
    """Pin to a wheel bundled inside the project, by a path relative to pyproject.toml."""
