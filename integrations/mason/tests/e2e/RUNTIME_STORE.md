...
and `LakebaseDurabilityStore` with an echo callback. No model endpoint, Session Store, Memory Store,
...
the body is `{"owner":{"app":{"name":"mason-...","service_principal_id":"<client UUID>"}}}`.
...
cp /tmp/mason-runtime-store-dist/databricks_mason-0.1.5.dev0-py3-none-any.whl /tmp/mason-runtime-store-app/
```
...
updated client. `agent.toml` enables durability and disables tracing; do not run this fixture
...
.venv/bin/python tests/e2e/runtime_store_check.py --profile "$MASON_TEST_PROFILE" --app mason-runtime-probe-a --invocation-id 4f1f0e53-71f1-4f90-8900-85fb2aa29231 --output /tmp/runtime-store-before.json
```
...
```bash
.venv/bin/mason --profile "$MASON_TEST_PROFILE" deployments stop mason-runtime-probe-a --yes
.venv/bin/mason --profile "$MASON_TEST_PROFILE" deployments start mason-runtime-probe-a
.venv/bin/python tests/e2e/runtime_store_check.py --profile "$MASON_TEST_PROFILE" --app mason-runtime-probe-a --invocation-id 4f1f0e53-71f1-4f90-8900-85fb2aa29231 --read-only --output /tmp/runtime-store-after.json
```
...
```bash
.venv/bin/mason --profile "$MASON_TEST_PROFILE" deployments delete mason-runtime-probe-a --yes
```
...
test aliases, with canonical APIProxy routing still pending. For subsequent runs, record
