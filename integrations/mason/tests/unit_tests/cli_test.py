...
def test_bad_option_uses_diagnostic_grammar():
    ...
    assert result.exit_code != 0
    assert "error: No such option '--nope'" in result.output
...
def test_root_help_dims_capabilities_block_not_surrounding_prose():
    import click
    ...
    # The capabilities rows recede as secondary text.
