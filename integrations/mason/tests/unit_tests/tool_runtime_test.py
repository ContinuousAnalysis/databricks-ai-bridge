"""Behavior tests for the Mason SDK's LangGraph Python-tool runtime."""
...
def _mcp_project(tmp_path: pathlib.Path) -> pathlib.Path:
    ...
    template_mason = _MASON_ROOT / "templates" / "agent-langgraph" / "agent" / "mason"
...
def _load_mcp_runtime(project: pathlib.Path, monkeypatch):
...
def test_langgraph_runtime_loads_direct_manifest_and_protects_sandbox_meta(
    ...
    runtime = _load_mcp_runtime(project, monkeypatch)
