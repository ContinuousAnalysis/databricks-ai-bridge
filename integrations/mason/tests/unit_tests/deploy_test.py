...
class _FakeClient:
    ...
    def create_runtime_store(
        self, runtime_store_id, app_service_principal_id, *, app_name, **kwargs
    ):
        return _runtime_store_response(runtime_store_id, app_name, app_service_principal_id)

    def get_runtime_store(self, name):
        raise AgentCliError("Runtime Store not found", error_code="NOT_FOUND")
...
def test_reconcile_durability_store_rejects_existing_store_owned_by_another_app(app_name, sp):
    client = mock.Mock()
    client.create_runtime_store.side_effect = AgentCliError("exists", error_code="ALREADY_EXISTS")
    store_id = deploy_mod.lakebase_durability_store.runtime_store_id("mason-myapp", "sp-123")
    ...
    with pytest.raises(AgentCliError, match="does not belong to this app identity"):
        deploy_mod._reconcile_durability_store(client, "mason-myapp", "sp-123")


@pytest.mark.parametrize("operation", ["create", "get"])
def test_reconcile_durability_store_propagates_api_failure(operation):
    client = mock.Mock()
...
def test_reconcile_and_delete_legacy_store_require_the_same_sp(legacy_name):
    ...
    backend = deploy_mod._reconcile_durability_store(client, "mason-myapp", "sp-123")
    assert backend.database == resource["storage_backend"]["lakebase"]["database_id"]
    deploy_mod._delete_durability_store(client, "mason-myapp", "sp-123")
...
def test_deploy_durability_binding_uses_dedicated_backend_with_session_store(
    ...
    store_id = deploy_mod.lakebase_durability_store.runtime_store_id("mason-myapp", "sp-123")
    backend = deploy_mod.lakebase_durability_store.backend_from_api(
        "mason-myapp", store_id, "sp-123", _runtime_store_response(store_id)
    )
