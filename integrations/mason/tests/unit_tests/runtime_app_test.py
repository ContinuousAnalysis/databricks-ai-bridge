...
async def running_client(app: AgentApp) -> AsyncIterator[httpx.AsyncClient]:
    await app._runtime.start(recover=app._recover_hook is not None)
    try:
...
def test_app_is_asgi_app_with_instance_scoped_decorators() -> None:
    ...
    assert app._recover_hook is recover
...
def test_agent_app_defaults_to_process_local_state_outside_apps(monkeypatch) -> None:
    ...
    assert app.runtime_store_persistent is False
    assert isinstance(app._runtime.durability_store, InMemoryRuntimeStore)


def test_deployed_app_without_runtime_store_resource_fails_startup(monkeypatch) -> None:
    monkeypatch.delenv(RUNTIME_LOCAL_ENV, raising=False)
