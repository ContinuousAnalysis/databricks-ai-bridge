...
def test_template_ref_pins_versioned_template_to_release_tag(monkeypatch: pytest.MonkeyPatch):
    ...
    assert init_mod._template_ref(init_mod._TEMPLATES["langgraph"]) == "databricks-mason-v0.3.0"
    # An unversioned template (different repo, not tagged in lockstep) keeps its default ref.
    assert init_mod._template_ref(init_mod._TEMPLATES["openai"]) == "main"
...
def test_template_ref_falls_back_to_main_for_unreleased_builds(
    ...
    assert init_mod._template_ref(init_mod._TEMPLATES["langgraph"]) == "main"
