...
distribution remains `databricks-mason`; installing it provides the `ab` command and AgentKit.
...
| **SDK / runtime** | `databricks_agentkit.AgentKitClient`, `databricks_mason.runtime`, the `langgraph`/`openai` adapters, `DurableAgentServer` | a scaffold depends on the **released** `databricks-mason` distribution from PyPI; opt into local or unreleased code with a `[tool.uv.sources]` override (see below) |
...
A scaffold uses a normal `databricks-mason` PyPI dependency, so `ab dev` and `ab deploy`
